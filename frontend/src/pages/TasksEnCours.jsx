...
function TasksEnCours() {
  ...
  const fetchTasks = async () => {
    try {
      ...
      setTasks(data.filter((t) => t.status === "En cours"));
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (updatedTask.status !== "En cours") {
    ...
    <div className="task-page-layout">
      ...
      <div className="task-page-content">
        <div className="tasks-list-container">
          <h1 className="page-title">Tâches en cours</h1>

          <div className="tasks-wrapper">
            {tasks.length === 0 ? (
              <p className="empty-message">Aucune tâche en cours pour le moment.</p>
            ) : (
              tasks.map((task) => (
                <div
                    key={task._id}
