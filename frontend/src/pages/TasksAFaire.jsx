...
function TasksAFaire() {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const filtered = data.filter((t) => t.status === "À faire");
      setTasks(filtered);
    } catch (error) {
      console.error("Erreur chargement tâches à faire", error);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      // If status changed to something else, remove it from this list
      if (updatedTask.status !== "À faire") {
      ...
      <div className="task-page-content">
        <div className="tasks-list-container">
          ...
          <div className="tasks-wrapper">
            {tasks.length === 0 ? (
              <p className="empty-message">Aucune tâche à faire</p>
            ) : (
              tasks.map((task) => (
                <div
                  key={task._id}
